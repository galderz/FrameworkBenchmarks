...
WORKDIR /quarkus-epoll
COPY --from=maven /quarkus-epoll/hibernate/target/lib lib
COPY --from=maven /quarkus-epoll/hibernate/target/hibernate-1.0-SNAPSHOT-runner.jar app.jar
